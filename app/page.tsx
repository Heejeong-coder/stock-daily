...
interface Realtime {
  ...
  kospi_change: string
  usdkrw: string
  samsung_price: string
  samsung_change: string
  hynix_price: string
  hynix_change: string
  kia_price: string
  kia_change: string
...
function ChangeTag({ change }: { change: string }) {
  ...
  const arrow = num >= 0 ? '▲' : '▼'
  return <span style={{ color, fontSize: 11, fontFamily: 'var(--font-jetbrains)' }}>{arrow}{Math.abs(num)}%</span>
}
...
export default function Home() {
  ...
  return (
    <div style={{ position: 'relative', zIndex: 1, maxWidth: 760, margin: '0 auto', padding: '40px 20px 80px' }}>
      ...
      {/* 헤더 */}
      <div style={{ textAlign: 'center', marginBottom: 32, paddingBottom: 24, borderBottom: '1px solid #2a2a3a' }}>
        <div style={{ fontFamily: 'var(--font-jetbrains)', fontSize: 11, color: '#c8a96e', letterSpacing: 3, marginBottom: 12 }}>주식 기초 마스터 클래스</div>
        <h1 style={{ fontSize: 'clamp(26px,5vw,40px)', fontWeight: 700, lineHeight: 1.2, marginBottom: 8 }}>
          매일 아침 <span style={{ color: '#c8a96e' }}>한 레슨</span>
        </h1>
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginTop: 12, background: 'rgba(200,169,110,0.1)', border: '1px solid rgba(200,169,110,0.3)', borderRadius: 4, padding: '6px 14px', fontFamily: 'var(--font-jetbrains)', fontSize: 12, color: '#c8a96e' }}>
      ...
      {realtime && (
        <div style={{ background: '#111118', border: '1px solid #2a2a3a', borderRadius: 12, padding: '16px 20px', marginBottom: 24 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(130px,1fr))', gap: 12 }}>
            {[
              { label: '코스피', price: realtime.kospi_price, change: realtime.kospi_change },
              { label: '원/달러', price: `₩${realtime.usdkrw}`, change: null },
              { label: '삼성전자', price: `₩${realtime.samsung_price}`, change: realtime.samsung_change },
              { label: 'SK하이닉스', price: `₩${realtime.hynix_price}`, change: realtime.hynix_change },
              { label: '기아', price: `₩${realtime.kia_price}`, change: realtime.kia_change },
            ].map((item) => (
              <div key={item.label} style={{ background: '#1a1a24', borderRadius: 8, padding: '10px 12px' }}>
                <div style={{ fontSize: 11, color: '#888899', marginBottom: 4 }}>{item.label}</div>
                <div style={{ fontFamily: 'var(--font-jetbrains)', fontSize: 14, fontWeight: 600, marginBottom: 2 }}>{item.price}</div>
      ...
      {!loading && activeTab === 'brief' && (
        <div className="animate-fade-up">
          {!brief ? (
            <div style={{ textAlign:'center', padding:'80px 0', color:'#888899' }}>
              <div style={{ fontSize:40, marginBottom:16 }}>📊</div>
              <div>오늘 브리핑이 준비 중입니다</div>
            </div>
          ) : (
            <>
              <div style={{ background:'#111118', border:'1px solid #2a2a3a', borderRadius:12, padding:24, marginBottom:16 }}>
              ...
              <div style={{ background:'linear-gradient(135deg,rgba(200,169,110,0.06),rgba(78,205,196,0.04))', border:'1px solid rgba(200,169,110,0.2)', borderRadius:12, padding:24 }}>
